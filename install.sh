...
if command -v apt-get &>/dev/null; then
    ...
    echo "ERROR: unsupported distro (no apt/pacman/dnf found)"
    exit 1
fi

echo "[1/4] Installing system packages ($PKG_MGR)..."
...
case "$PKG_MGR" in
    apt)
        ...
        # Try to install it; if it fails, warn the user.
...
pip3 install --user dbus-next evdev
...
echo "  - Press your hotkey (default F9) AFTER something happens."
